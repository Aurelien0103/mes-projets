<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link  rel="stylesheet" href="facture.css">
    <title>Apple</title>
</head>
<body>
    <div class="logo-center">
    <img src="data:image/png;base64," class="logo">
    </div>

    <div class="coords">
        Apple Inc.<br>
        1 Infinite Loop<br>
        Cupertino, CA 95014<br>
        USA<br>
        +1 (408) 996-1010
    </div>

<h2>Facture</h2>

<table>
    <tr>
        <th>Description</th>
        <th>Quantité</th>
        <th>Prix unitaire</th>
        <th>Total</th>
    </tr>
        <?php
        $client = array(
            "nom" => "Tim Cook",
            "adresse" => "123 Main St",
            "ville" => "San Francisco",
            "code_postal" => "94114",
            "pays" => "USA"
        );

        $prestations = array(
            array(
                "description" => "iMac Pro",
                "quantite" => mt_rand(1,5),
                "prix_unitaire" => 4999.99
            ),
            array(
                "description" => "iPhone 13",
                "quantite" => mt_rand(1,5),
                "prix_unitaire" => 999.99
            ),
            array(
                "description" => "Apple Watch",
                "quantite" => mt_rand(1,5),
                "prix_unitaire" => 129.99
            )
        );

        $tva = 0.2;
        $total_ht = 0;
        foreach($prestations as $prestation) {
            $total_ligne = $prestation["quantite"] * $prestation["prix_unitaire"];
            $total_ht += $total_ligne;
            echo "<tr>";
                    echo "<td>".$prestation["description"]."</td>";
                    echo "<td>".$prestation["quantite"]."</td>";
                    echo "<td>".$prestation["prix_unitaire"]."</td>";
                    echo "<td>".$total_ligne."</td>";
                    echo "</tr>";
                }
        $total_ttc = $total_ht + $total_ht * $tva;
        ?>
    </table>
        
    <div class="total">
        <p>Total HT : <?php echo $total_ht; ?> &euro;</p>
        <p>TVA (20%) : <?php echo $total_ht * $tva; ?> &euro;</p>
        <p>Total TTC : <?php echo $total_ttc; ?> &euro;</p>
    </div>

    <div class="facture-emise">
    <p>Facture émise pour :</p>
    <p><?php echo $client["nom"]; ?></p>
    <p><?php echo $client["adresse"]; ?></p>
    <p><?php echo $client["ville"]." ".$client["code_postal"]; ?></p>
    <p><?php echo $client["pays"]; ?></p>
    </div>
    <div class="mention">
    <p>Mentions obligatoires en France :</p>
    <p>Société immatriculée au RCS de Paris sous le numéro 9274563</p>
    <p>N° TVA Intracommunautaire : FR 12 345 678 901</p>
    </div>
</body>
</html>